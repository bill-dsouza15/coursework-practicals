...
bool graphcoloring(int** graph, int n, int m, int* res, int v){
    ...
    fors(i,0,m){
/*      
        Prints the stage wise output for each vertice
        fors(i,0,n){
            cout<<i<<" - "<<res[i]<<" | ";
        }
        cout<<endl;
...
int main()
{
    ...
    //We can check for valid m using Brook's theorem
    ...
    while(true){
        ...
        if(m > 0){
            if(m <= (Delta+1)){
                ...
                cout<<"Violation : Expected m <= Delta(G) + 1"<<endl;
            }
        }
        else{
