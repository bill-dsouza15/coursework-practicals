...
int main(int argc, char** argv){
    ...
    stack<map<string,string>> arglistmap;               //Stack of maps for nested macro
    ...
    while(getline(file,cmdline)){
        ...
        if(sParse[0].compare("MACRO") == 0){
            ...
            if(d>1){
                mdt.pb(make_pair(arglistmap.top()[sParse[1]],sParse[2]));
            }
            else{
                mdt.pb(make_pair(sParse[1],sParse[2]));
            }
            arglistm = macroHeader(sParse, arglistm);
            arglistmap.push(arglistm);
            arglistm.clear();
            location++;
        }
        ...
        else
        {
            if(d>=1){
                string param="";
                param = macroBody(param, sParse, arglistmap.top());
    ...
    for (auto itr : mnt){
        cout<<itr.first;
        if(itr.first.length() < 10){
            fori(0 , (10 - itr.first.length())){
                cout<<" ";
            }
        }
        cout<<"  "<<itr.second<<endl;
